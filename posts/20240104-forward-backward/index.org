...
with a greedy bootstrap target. Also, this is, roughly, exactly what
the FB algorithm is doing.
...
function encoder". Under this model, we have
...
optimal policies and the $\bar{F}$ factor by the "reward
...
approximators. The paper shows some conditions on when such a
