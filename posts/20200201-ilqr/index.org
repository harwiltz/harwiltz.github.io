...
* iLQR Without Obfuscation
/Harley Wiltzer, February 2020/
** What's Wrong With the iLQR Literature?
  :PROPERTIES:
  :CUSTOM_ID: whats-wrong-with-the-ilqr-literature
  :END:
...
motivate iLQR is /almost/ like using SAT to motivate integer
...
/trajectory optimization/ for nonlinear systems. Essentially, the
...
- The cost function will be approximated with second degree Taylor
...
So basically we need to figure out what =ForwardRollout= and
=BackwardUpdate= do. Readers that are familiar with machine learning
...
*** Some Notation and Modeling
   :PROPERTIES:
   :CUSTOM_ID: some-notation-and-modeling
   :END:
...
*** Putting the "Optimization" in Trajectory Optimization
   :PROPERTIES:
   :CUSTOM_ID: putting-the-optimization-in-trajectory-optimization
   :END:
...
controls in a /proposed/ trajectory, while \(x_t, u_t\) refer to states
and controls in the trajectory that is being improved (referring to the
pseudocode above, \(x, u\) would be in =states= and =controls=, while
\(x_t, u_t\) would be in =state_trajectory= and =control_trajectory=).
...
\(K_t\) and \(d_t\) vectors. This is the job of =BackwardUpdate= in the
pseudocode above.

** Generating Rollouts
   :PROPERTIES:
   :CUSTOM_ID: generating-rollouts
   :END:
With the =BackwardUpdate=, we know how to adjust controls given some
error in state. Now we discuss how this is implemented in iLQR -- this
is the =ForwardRollout= subroutine described in the pseudocode above.

Recall that =ForwardRollout= takes as input a trajectory of states, a
...
[[#the-initial-trajectory][how to initialize the first rollout later]],
and the trajectory of controls corresponds to the controls used in the
previous trajectory. Now, =ForwardRollout= works as follows:

- Let \(\bar{x}_t\) denote the previous state trajectory, \(\bar{u}_t\)
  the previous control trajectory
...
- For \(t\) from \(0\) to \(T-1\)
  ...
  2. Compute \(\delta u_t\) using \eqref{eq:du}:
     \(\delta u_t\leftarrow K_t\delta x_t + d_t\)
  3. Update controls: \(u_t\leftarrow \bar{u}_t + \delta u_t\)
  4. Aggregate loss: \(L\leftarrow L + x_t^\top Qx_t + u_t^\top u_t\)
  5. Get next state from dynamics: \(x_{t+1}\leftarrow f(x_t, u_t)\)
...
** Implementation Details and Tricks
  :PROPERTIES:
  :CUSTOM_ID: implementation-details-and-tricks
  :END:
...
*** The Initial Trajectory
   :PROPERTIES:
   :CUSTOM_ID: the-initial-trajectory
   :END:
...
- \(u_t = 0\): This one was suggested by Li and Todorov (the inventors
  of iLQR). Intuitively, this might make training a little slow if the
  starting state is in a stable equilibrium.
- \(u_t = \alpha\sin(\omega t), \alpha,\omega\in\mathbf{R}\): This is
...
- \(u_t\sim\mathcal{N}(0, \Sigma), \Sigma=\Sigma^\top\in\mathbf{R}^{n_u\times
  ...
  the best during my experiments.
...
*** Gradient Clipping
   :PROPERTIES:
   :CUSTOM_ID: gradient-clipping
   :END:
I cannot emphasize enough how *essential* this trick was. When tuning my
...
Upon employing this trick, these divergence issues ceased *immediately*.
...
This trick /does/ impose an additional hyperparameter, however in my
...
effect of this, but without forcing /very/ small step sizes, this can't
...
*** Scaling Cost Function Weights
   :PROPERTIES:
   :CUSTOM_ID: scaling-cost-function-weights
   :END:
...
*** Computing Jacobians the Lazy Way
   :PROPERTIES:
   :CUSTOM_ID: computing-jacobians-the-lazy-way
   :END:
...
1. The state-control space is large, so deriving the Jacobians could be
   very time consuming and/or error-prone (or you're simply too lazy to
   derive the Jacobians)
2. You don't actually have an explicit dynamics model (for instance,
   perhaps you have a powerful simulator instead)

Fortunately, we can automatically approximate Jacobians using the method
of [[https://en.wikipedia.org/wiki/Finite_difference_method][finite
differences]], which is so easy to implement that it's almost
...
1. [[https://homes.cs.washington.edu/~todorov/papers/LiICINCO04.pdf][Li
   and Todorov -- Iterative Linear Quadratic Regulator Design for
   Nonlinear Biological Movement Systems]]
2. [[https://rexlab.stanford.edu/papers/iLQR_Tutorial.pdf][Jackson and
   Howell -- iLQR Tutorial]]
3. [[https://studywolf.wordpress.com/2016/02/03/the-iterative-linear-quadratic-regulator-method/][Travis
   DeWolf -- The Iterative Linear Quadratic Regulator Algorithm]]
